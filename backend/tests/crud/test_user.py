...
from werefa.shared.models import User, UserCreate, UserUpdate
...
def test_authenticate_user(db: Session) -> None:
    ...
    user_in = UserCreate(email=email, password=password)
    user = identity_repo.create_user(session=db, user_create=user_in)
    authenticated_user = identity_repo.authenticate(
...
def test_not_authenticate_user(db: Session) -> None:
    email = random_email()
    password = random_lower_string()
    user = identity_repo.authenticate(session=db, email=email, password=password)
    assert user is None
...
def test_update_user(db: Session) -> None:
    ...
    if user.id is not None:
        identity_repo.update_user(session=db, db_user=user, user_in=user_in_update)
    user_2 = db.get(User, user.id)
    assert user_2
    assert user.email == user_2.email
